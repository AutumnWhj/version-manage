...
ARG VERSION=1638861811436
ARG OSS_SECRET_ID
ARG OSS_SECRET_KEY
ARG OSS_BUCKET
ARG OSS_REGION

RUN coscmd config -a ${OSS_SECRET_ID} -s ${OSS_SECRET_KEY} -b ${OSS_BUCKET} -r ${OSS_REGION} && coscmd download -rf html2email/release/test/default /dist
